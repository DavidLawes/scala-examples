...
class HuffmanSuite extends munit.FunSuite:
  ...
  test("times") {
    assertEquals(times(List('a', 'b', 'a')), List(('b', 1), ('a', 2)))
  }
  ...
  test("create code tree") {
    ...
    assertEquals(createCodeTree(chars), Fork(Fork(Leaf('c', 1), Leaf('b', 1), List('c', 'b'), 2), Leaf('a', 2), List('c', 'b', 'a'), 4))
  }
  ...
  test("quick encode") {
    new TestTrees:
      ...
      assertEquals(quickEncode(t2)("aabbddd".toList), List(0, 0, 0, 0, 0, 1, 0, 1, 1, 1, 1)) 
  }
